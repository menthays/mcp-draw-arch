...
const { SemanticEngine } = require('./engine');
...
class SemanticArchitectureGenerator {
  constructor() {
    ...
    this.semanticEngine = new SemanticEngine(ARCHITECTURE_PATTERNS);
  }
  ...
  /**
   * Generate architecture diagram from a natural language description.
   * @param {string} description - The user's natural language text.
   * @returns {Object} Excalidraw diagram data.
   */
  async generateFromDescription(description) {
    console.log('New flow: Generating from natural language description...');
    // Step 1: Analyze the text to get a structured architecture model
    const architectureDescription = this.semanticEngine.analyze(description);
    
    // Step 2: Generate the diagram from the structured model
